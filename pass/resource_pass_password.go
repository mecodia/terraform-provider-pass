...
func passPasswordResourceWrite(d *schema.ResourceData, meta interface{}) error {
	...
	if (password != "" || len(data) != 0) && yaml != "" {
		return errors.New("can't set password/data and yaml at the same time")
	}
	...
	if yaml != "" {
		...
		tmpl, err := template.New("secret").Parse(`{{ .Password}}
{{ range $key, $value := .Data }}
{{ $key }}: {{ $value }}
{{ end }}
`)
		err = tmpl.Execute(&result, &templateData{password, data})
		if err != nil {
